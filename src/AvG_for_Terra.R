...
before_opt_results <- A$Results_ReScore$[...] %>% select([...]) %>% distinct()
after_opt_results <- A$Results_ReScore$[...] %>% select([...]) %>% distinct()
...
write.table(before_opt_results,file=paste0(output_dir,"/Report_GR_Replicate_",Name_Tag,"_",i,".csv"),quote=F,row.names=F,col.names=F,sep=";")
write.table(after_opt_results,file=paste0(output_dir,"/Report_GR_ReScore_",Name_Tag,"_",i,".csv"),quote=F,row.names=F,col.names=F,sep=";")